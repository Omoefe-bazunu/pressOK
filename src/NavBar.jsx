...
const NavBar = () => {
  ...
  // DOWNLOAD LOGO IMAGE IMAGE
  const [logoImage, setLogoImage] = useState("");

  useEffect(() => {
    // Function to fetch the image from Firebase Storage
    const fetchImage = async () => {
      const logoRef = ref(storage, "gen/logo.png");
      try {
        const url = await getDownloadURL(logoRef);
        setLogoImage(url); // Set the downloaded image URL as the background
      } catch (error) {
  ...
  return (
    <div className="navBar w-full h-20 border-b-2 border-gray-300 flex items-center justify-between px-24">
      <a href="/">
        <img
          src={logoImage}
          className="logo font-bold w-18 h-24 flex mt-4 items-center justify-center ml-12 text-xl"
        />
      </a>
      <ul className="flex desktoplink flex-row items-center cursor-pointer h-full">
        <a href="#Products" className=" h-full">
          <p className="navLinkItem h-full font-bold px-4 flex items-center justify-center hover:bg-neutral-400 hover:text-white ">
            PRODUCTS
          </p>
        </a>
        <a href="#Services" className=" h-full">
          <p className="navLinkItem h-full font-bold px-4 flex items-center justify-center hover:bg-neutral-400 hover:text-white">
            SERVICES
          </p>
        </a>
      ...
      {!toggle && (
        <div className="Bar flex flex-col scale-up-tr w-full h-fit py-8 absolute right-0 z-10 mt-80">
          <ul className="flex flex-col justify-start cursor-pointer py-2 w-full h-full text-white text-lg">
            <a
              href="#Products"
              className=" h-full px-8 hover:border-l-2 py-2 border-white hover:bg-gray-600"
            >
              <p>PRODUCTS</p>
            </a>
            <a
